function getGroupsComments() { //Получение комментов
    out.innerHTML = '';
    let posts_id_arr = [];
    let ids = uid.value;
    ...
    let text, group_id, comment_id, post_id, n, tag, otvet;
    let x = y = z = 0; // переменная для хранения номера массива
    let links_arr = [];
    let html = "Комментарии групп";
    for (let a = 0; a < ids_arr.length; a++){ //обработка разных сообществ
        sendRequest('wall.get', {count: 100, owner_id: -ids_arr[a]}, function (data) {	//получение постов со стены
            ...
            data = undefined;
            for(let b = 0; b < posts_id_arr.length; b++){
                    ...
                    top: for(let i = 0; i < data.response.items.length; i++){
                        ...
                        for (let j = 0; j < tags_arr.length; j++){ //цикл для проверки разных тегов
                            ...
                            if(n > 0){
                                ...
                                links_arr[x] = 'https://vk.com/wall' + group_id + '_' + posts_id_arr[b] + '?reply=' + comment_id;
                                x++;
